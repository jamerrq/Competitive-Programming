public class Chess
{
    ...
    public static void main(String[] args){
        ...
        for(int i = 0; i < n; i++){
            ...
            boolean status = true;
            if(board[8 - (mi.charAt(1) - 48)][mi.charAt(0) - 65] != board[8 - (mf.charAt(1) - 48)][mf.charAt(0) - 65]){
                status = false;
            }
            if(status){ // Same color
                if(mi2 == mf2){ // Same number
                    if(mi.charAt(0) == mf.charAt(0)){ // Same character
                        System.out.println(0 + " " + mi.charAt(0) + " " + mi.charAt(1));
                    }else{ // Different character
                        int c = (mi1.charAt(0) + mf1.charAt(0)) / 2; 
                        int h = (Math.abs((mf1.charAt(0) - 65) - (mi1.charAt(0) - 65)) + 2) / 2;
                        if(mi2 > 4)h = mi2 + 1 - h;
                        else h = mi2 + h - 1;
                        System.out.println(2 + " " + mi1 + " " + mi2 + " " + (char)c + " " + h + " " + mf1 + " " + mf2);
                    }
                }else{ // Different number
                    if(mi1.charAt(0) == mf1.charAt(0)){ // Same character
                        int r = (mi2 + mf2) / 2, w = (Math.abs(mf2 - mi2) + 2) / 2;
                        if(mi1.charAt(0) > 68)w = mi1.charAt(0) - w + 1;
                        else w += mi1.charAt(0) - 1;
                        System.out.println(2 + " " + mi1 + " "  + mi2 + " " + (char)w + " " + r + " " + mf1 + " " + mf2);
                    }else{ // Different character
                        if(Math.abs(mf2 - mi2) == Math.abs(mf1.charAt(0) - mi1.charAt(0))){  //Same diagonal
                            System.out.println(1 + " " + mi1 + " " + mi2 + " " + mf1 + " " + mf2);
                        }else{
                           int[] diagonals = diagonalsNW(mi1.charAt(0), mi2, mf1.charAt(0), mf2);
                           if(diagonals == null){diagonals = diagonalsNE(mi1.charAt(0), mi2, mf1.charAt(0), mf2);}
                           if(diagonals == null){diagonals = diagonalsSW(mi1.charAt(0), mi2, mf1.charAt(0), mf2);}
                           if(diagonals == null){diagonals = diagonalsSE(mi1.charAt(0), mi2, mf1.charAt(0), mf2);}
                           System.out.println(2 + " " + mi1 + " " + mi2 + " " + (char)diagonals[0] + " " + diagonals[1] + " " + mf1 + " " + mf2);
                        }
                    }
                }
